  ...
  const handleImport = async (book) => {
    ...
    try {
      if (user.role === "admin") {
        ...
        const existingBook = allBooksResponse.data.find((b) => b.isbn === book.isbn)

        if (!existingBook) {
          alert("This book is not available in the main collection. Only admin can add new books.")
          return
        }
  ...
  return (
    <div className="fade-in">
      <div className="card" style={{ marginBottom: "2rem" }}>
        <h1 style={{ fontSize: "2.5rem", fontWeight: 800, marginBottom: "1.5rem", background: "linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
           Browse All Books
        </h1>
        
        {/* Subject Filter */}
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label style={{ marginBottom: "0.5rem", display: "block", fontWeight: 600 }}>Filter by Subject:</label>
          <select
            value={subject}
      ...
      {totalItems > 0 && (
        <div className="card" style={{ marginBottom: "2rem", textAlign: "center" }}>
          <p style={{ fontSize: "1.1rem", color: "var(--gray)", fontWeight: 600 }}>
            Showing {currentPage * 20 + 1} - {Math.min((currentPage + 1) * 20, totalItems).toLocaleString()} of {totalItems.toLocaleString()} books
            {subject && ` in "${subject}"`}
          </p>
        </div>
      )}
      ...
      {books.length > 0 ? (
        <>
            ...
            {books.map((book) => (
              <div key={book.googleId} className="book-card">
                ...
                {book.averageRating > 0 && (
                  <p>
                    <strong>Rating:</strong> ⭐ {book.averageRating.toFixed(1)} / 5.0 ({book.ratingsCount} ratings)
                  </p>
                )}
                {book.description && (
                  <p style={{ fontSize: "14px", color: "#666", maxHeight: "100px", overflow: "hidden" }}>
          ...
          <div className="card" style={{ marginTop: "2rem", marginBottom: "2rem" }}>
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
              ...
              <span style={{ padding: "10px", display: "flex", alignItems: "center", fontWeight: 600, color: "var(--dark)" }}>
        ...
        !loading && (
          <div className="card" style={{ textAlign: "center", padding: "3rem" }}>
            <h2 style={{ marginBottom: "1rem", color: "var(--gray)" }}>No books found</h2>
            <p style={{ color: "var(--gray)" }}>Try selecting a different subject or check back later.</p>
          </div>
        )
      )}
    </div>
